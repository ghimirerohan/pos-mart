...
def fetch():
	...
	try:
		...
		for template in response["data"]:
			...
			flags = 1
			if frappe.db.exists("WhatsApp Message Templates", {"actual_name": template["name"]}):
				doc = frappe.get_doc("WhatsApp Message Templates", {"actual_name": template["name"]})
			else:
				flags = 0
				doc = frappe.new_doc("WhatsApp Message Templates")
